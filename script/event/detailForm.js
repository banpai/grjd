window.apiready=function(){
  var vm = new Vue({
    ...
    created:function(){
      ...
      this.workId = api.pageParam.workId;
      this.trackId = api.pageParam.trackId;
      UICore.showLoading('加载中...','稍等...');
      this.getForm();
    },
    methods:{
      getForm:function(){
        console.log(UICore.serviceUrl+'mobile/mobileWf.shtml?act=editEvent_HZ&state=VIEW&loginId='+this.accountId+'&workId=' + this.workId+'&trackId='+this.trackId);
        api.ajax({
          url : UICore.serviceUrl+'mobile/mobileWf.shtml?act=editEvent_HZ&state=VIEW&loginId='+this.accountId+'&workId=' + this.workId+'&trackId='+this.trackId,
          tag : 'grid',
          method : 'get'
        }, function(ret, err) {
          if (ret) {
