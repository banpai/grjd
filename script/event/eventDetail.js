window.apiready=function(){
  var vm = new Vue({
    ...
    methods:{
      loadFlow:function(){
        console.log(UICore.serviceUrl+'mobile/mobileWf.shtml?act=viewOpinion_HZ&workId=' + this.id+"&trackId="+this.trackId);
        UICore.showLoading('加载中...','稍等...');
        api.ajax({
    			url : UICore.serviceUrl+'mobile/mobileWf.shtml?act=viewOpinion_HZ&workId=' + this.id+"&trackId="+this.trackId,
    			tag: 'grid',
    			method : 'get'
    		}, function(ret, err) {
    			if (ret) {
      ...
      getForm:function(){
        console.log(UICore.serviceUrl+'mobile/mobileWf.shtml?act=editEvent_HZ&state=VIEW&loginId='+this.accountId+'&workId=' + this.id+"&trackId="+this.trackId);
        api.ajax({
          url : UICore.serviceUrl+'mobile/mobileWf.shtml?act=editEvent_HZ&state=VIEW&loginId='+this.accountId+'&workId=' + this.id+"&trackId="+this.trackId,
          tag : 'grid',
          method : 'get'
        }, function(ret, err) {
          if (ret) {
      ...
      nextExecutorClick:function(){
        ...
        console.log(UICore.serviceUrl+'mobile/mobileWf.shtml?act=getActivitySelect_HZ&state=isEdit&accountId='+this.accountId+'&workId='+this.id+"&trackId="+this.trackId);
        api.ajax({
          url : UICore.serviceUrl+'mobile/mobileWf.shtml?act=getActivitySelect_HZ&state=isEdit&accountId='+this.accountId+'&workId='+this.id+"&trackId="+this.trackId,
          method : 'get',
          tag:'grid'
        }, function(ret, err) {
          api.hideProgress();
      ...
      selectPerson:function(activityId,doSelectType){
        ...
        if(activityId.indexOf("End")>-1){
          ...
          api.ajax({
            url : UICore.serviceUrl+'mobile/mobileWf.shtml?act=selectActorsTree_HZ&state=isNew&nodeId='+activityId+'&accountId='+this.accountId+'&workId='+this.id+'&seclectId='+doSelectType+"&trackId="+this.trackId,
            tag:'grid',
            method : 'get'
          }, function(ret, erra) {
            api.hideProgress();
      ...
      submitForm:function(){
        ...
        api.ajax({
            url: UICore.serviceUrl+'mobile/mobileWf.shtml?act=submitEvent_HZ&accountId='+this.accountId+'&workId='+this.alltag.flowInstanceId+'&seclectId='+this.alltag.selectId+"&trackId="+this.trackId,
            method: 'post',
            tag:'grid',
            data: {
