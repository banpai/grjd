...
apiready = function() {
    var vue = new Vue({
        ...
        created: function() {
            ...
            var that = this;
            console.log(UICore.serviceUrl + 'mobile/mobileInterfaceForBuilding.shtml?act=query&accountId='+this.accountId+'&data=' + JSON.stringify(this.resultjson));
            api.ajax({
                url: UICore.serviceUrl + 'mobile/mobileInterfaceForBuilding.shtml?act=query&accountId='+this.accountId+'&data=' + JSON.stringify(this.resultjson),
                method: 'get',
            }, function(ret, err) {
                api.hideProgress();
                if (ret.success) {
            ...
            api.addEventListener({
                ...
                if (ret) {
                    ...
                    for(var num in _self.items){
                      if(_self.items[num].id==myJson.Building.id){
                        for(mykey in myJson.Building){
                          if(mykey=="ADDRESS"){
                            _self.items[num]["address"] = myJson.Building[mykey];
                          }else if(mykey=="BUI_NAME"){
                            _self.items[num]["BUI_NAME"] = myJson.Building[mykey];
                          }else if(mykey=="BUI_CODE"){
                            _self.items[num]["buiCode"] = myJson.Building[mykey];
                          }else if(mykey=="ENTITYID"){
                            _self.items[num]["entityId"] = myJson.Building[mykey];
                          }
        ...
        methods: {
            ...
            loadList: function() {
                ...
                console.log("下拉刷新: " + UICore.serviceUrl + 'mobile/mobileInterfaceForBuilding.shtml?act=query&accountId='+this.accountId+'&data=' + JSON.stringify(this.resultjson));
                api.ajax({
                    url: UICore.serviceUrl + 'mobile/mobileInterfaceForBuilding.shtml?act=query&accountId='+this.accountId+'&data=' + JSON.stringify(this.resultjson),
                    method: 'get',
                }, function(ret, err) {
            ...
            loadBottom: function() { //上拉加载
                ...
                api.addEventListener({
                    ...
                    console.log(UICore.serviceUrl + 'mobile/mobileInterfaceForBuilding.shtml?act=query&accountId='+this.accountId+'&data=' + JSON.stringify(that.resultjson));
                    api.ajax({
                        url: UICore.serviceUrl + 'mobile/mobileInterfaceForBuilding.shtml?act=query&accountId='+this.accountId+'&data=' + JSON.stringify(that.resultjson),
                        method: 'get',
                    }, function(ret, err) {
                        api.hideProgress();
                        if (ret.success) {
            ...
            query: function() {
                ...
                console.log(UICore.serviceUrl + 'mobile/mobileInterfaceForBuilding.shtml?act=query&accountId='+this.accountId+'&data=' + JSON.stringify(resultjson));
                api.ajax({
                    url: UICore.serviceUrl + 'mobile/mobileInterfaceForBuilding.shtml?act=query&accountId='+this.accountId+'&data=' + JSON.stringify(resultjson),
                    method: 'get',
                }, function(ret, err) {
                    api.hideProgress();
                    that.isshow = false;
