...
apiready = function() {
    new Vue({
        ...
        created: function() {
            ...
            if (this.isBrowser) {
                this.edited = this.params.edited;
                ...
                api.ajax({
                    ...
                    if (ret) {
                        if (ret.success) {
                            ...
                            if (ret.data.completedTime.length > 10) {
                                that.buildingTime = ret.data.completedTime.substring(0, 10);
                            } else {
        ...
        methods: {
            ...
            mapNumf: function() { //地图编号
                ...
                api.addEventListener({
                    ...
                    if (ret) {
                        ...
                        that.mapNum = ret.value.mapNum;
                        that.buildingName = ret.value.building_name;
                        that.address = ret.value.address;
                    } else {
                        alert(JSON.stringify(err));
                    }
                });
            ...
            coveredAreaf: function() {
                var Areas = this.coverArea;
                if (!(/^[1-9]\d*$/.test(Areas))) {
                    api.toast({
                        msg: '请输入正确的建筑面积',
                        duration: 3000,
            ...
            submit: function() {
                ...
                if (this.isBrowser) {
                    ...
                    if (this.code!= "") {
                      if (this.code && this.checkNumber()) {
                          postjson.code = this.code;
                      }
                    }else{
                      alert("请输入建筑编号");
                      return;
                    ...
                    if (!this.address == "" || !this.address == null) {
                        postjson.ADDRESS = this.address;
                    } else {
                        alert("请输入地址");
                        return false;
                    };
                    // postjson.ADDRESS = this.address; //地址
                    if (!this.buildingName == "" || !this.buildingName == null) {
                        postjson.BUI_NAME = this.buildingName;
                    } else {
                        alert("请输入建筑名称");
                        return false;
                    };
                    ...
                    if (!this.buildingType == "" || !this.buildingType == null) {
                        postjson.buildingType = this.buildingType;
                    } else {
                        alert("请选择楼号类型");
                        return false;
                    };

                    if (!this.buildingNum == "" || !this.buildingNum == null && this.buildingType == "原有楼号") { //楼号类型
                    ...
                    //     return;
                    // }
                    if (!this.mapNum == "" || !this.mapNum == null) {
                        postjson.ENTITYID = this.mapNum;
                    } else {
                        alert("请选择地图编号");
                        return false;
                    };
                    ...
                    if (this.editUnit==""||parseInt(this.editUnit)<=0) {
                        alert("请输入编辑单元");
                      return false;
                    }
