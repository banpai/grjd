    ...
    new Vue({
        ...
        methods: {
            initSelectData: function () {
                ...
                var _self = this;
                console.log(UICore.serviceUrl + 'mobile/mobileCommunCorrect.shtml?act=getDetailInfo&data=' + JSON.stringify(json));
                api.ajax({
                    url: UICore.serviceUrl + 'mobile/mobileCommunCorrect.shtml?act=getDetailInfo&data=' + JSON.stringify(json),
                    method: 'post',
                }, function (ret, err) {
                    api.hideProgress();
                    if (ret) {
                        ...
                        if (ret.success) {
                            _self.ids = ret.data.id,
                                _self.name = ret.data.name, //姓名
                                _self.numberId = ret.data.numberId, //身份证号
                                _self.sex = ret.data.sex, //性别
                                _self.oldName = ret.data.oldName,//曾用名
                                _self.date = ret.data.date, //出生日期
                                _self.nation = ret.data.nation,//名族
                                _self.placeOrigin = ret.data.placeOrigin,//籍贯
                                _self.marry = ret.data.marry,//婚姻状况
                                _self.politicalLandscape = ret.data.politicalLandscape,//政治面貌

                                _self.education = ret.data.education, //文化程度
                                _self.religiousBelief = ret.data.religiousBelief, //宗教信仰
                                _self.workCategory = ret.data.workCategory, //职业类别
                                _self.work = ret.data.work, //职业
                                _self.unitService = ret.data.unitService,//	服务处所
                                _self.tel = ret.data.tel,//	联系方式
                                _self.domicile = ret.data.domicile,//	户籍地
                                _self.domicileAddress = ret.data.domicileAddress,//	户籍详细地址
                                _self.address = ret.data.address,//	现住地
                                _self.addressDetail = ret.data.addressDetail,//	现住地详细地址
                                _self.personnelNumber = ret.data.personnelNumber,//	社区矫正人员编号
                                _self.placeDetention = ret.data.placeDetention,//	原羁押场所
                                _self.correctiveCategory = ret.data.correctiveCategory,//	矫正类别
                                _self.caseCategory = ret.data.caseCategory,//	案件类别
                                _self.specificCharges = ret.data.specificCharges,//	具体罪名
                                _self.time = ret.data.time,//	原判刑期
                                _self.sentencingPeriod = ret.data.sentencingPeriod,//	原判刑开始日期
                                _self.sentencinOldgperiod = ret.data.sentencinOldgperiod,//	原判刑结束日期
                                _self.startData = ret.data.startData,//	矫正开始日期
                                _self.oldData = ret.data.oldData,//	矫正结束日期
                                _self.receivingMode = ret.data.receivingMode,//	接收方式
                                _self.fourCases = ret.data.fourCases,//	四史情况
                                _self.whetherRecidivists = ret.data.whetherRecidivists,//	是否累惯犯
                                _self.threeInvolved = ret.data.threeInvolved,//	三涉情况
                                _self.correctionGroup = ret.data.correctionGroup,//	是否建立矫正小组
                                _self.thereisFthecamp = ret.data.thereisFthecamp,//	是否有脱营
                                _self.reasonpPeremoval = ret.data.reasonpPeremoval,//	脱管原因
                                _self.checkSupervision = ret.data.checkSupervision,//	检查监督脱管情况
                                _self.rectifySituation = ret.data.rectifySituation,//	脱管纠正情况
                                _self.correctionDetail = ret.data.correctionDetail,//	矫正小组人员组成情况
                                _self.releaseType = ret.data.releaseType,//	矫正解除（终止）类型
                                _self.palatabilityType = ret.data.palatabilityType,//	适口类型
                                _self.isLouguan = ret.data.isLouguan,//	是否有漏管
                                _self.leakageReason = ret.data.leakageReason,//	漏管原因
                                _self.checkLeakage = ret.data.checkLeakage,//	检查监督漏管情况
                                _self.leakageCorrection = ret.data.leakageCorrection,//	漏管纠正情况
                                _self.punishments = ret.data.punishments,//	奖惩情况
                                _self.penaltyAlteration = ret.data.penaltyAlteration,//	刑罚变更执行情况
                                _self.isMorecrimes = ret.data.isMorecrimes,//	是否重新犯罪
                                _self.crimesName = ret.data.crimesName,//	重新犯罪名称犯罪
                                _self.otherProblem = ret.data.otherProblem//	其它需要说明的问题
                                _self.coordinate = ret.data.coordinate//关联地图
                            ...
                            for (var num in _self.marry_arr) {
                                if (_self.marry_arr[num].key == ret.data.marry) {
                                    _self.marry = _self.marry_arr[num].text;
                                    _self.marryId = _self.marry_arr[num].key;
                                }
                            }
                            for (var num in _self.politicalLandscape_arr) {
                                if (_self.politicalLandscape_arr[num].key == ret.data.politicalLandscape) {
                                    _self.politicalLandscape = _self.politicalLandscape_arr[num].text;
                                    _self.politicalLandscapeId = _self.politicalLandscape_arr[num].key;
                                }
                            }
                            ...
                            for (var num in _self.religiousBelief_arr) {
                                if (_self.religiousBelief_arr[num].key == ret.data.religiousBelief) {
                                    _self.religiousBelief = _self.religiousBelief_arr[num].text;
                                    _self.religiousBeliefId = _self.religiousBelief_arr[num].key;
                                }
                            }
                            for (var num in _self.workCategory_arr) {
                                if (_self.workCategory_arr[num].key == ret.data.workCategory) {
                                    _self.workCategory = _self.workCategory_arr[num].text;
                                    _self.workCategoryId = _self.workCategory_arr[num].key;
                                }
                            }
                            for (var num in _self.receivingMode_arr) {
                                if (_self.receivingMode_arr[num].key == ret.data.receivingMode) {
                                    _self.receivingMode = _self.receivingMode_arr[num].text;
                                    _self.receivingModeId = _self.receivingMode_arr[num].key;
                                }
                            }
                            ...
                            for (var num in _self.correctiveCategory_arr) {
                                if (_self.correctiveCategory_arr[num].key == ret.data.correctiveCategory) {
                                    _self.correctiveCategory = _self.correctiveCategory_arr[num].text;
                                    _self.correctiveCategoryId = _self.correctiveCategory_arr[num].key;
                                }
                            }
                            for (var num in _self.sex_arr) {
                                if (_self.sex_arr[num].key == ret.data.sex) {
                                    _self.sex = _self.sex_arr[num].text;
                                    _self.sexId = _self.sex_arr[num].key;
                                }
                            }
                            //多选，复选 四史情况
                            for (var num in _self.fourCases_arr) {
                                var temp_arr = ret.data.fourCases.split(',');
                                //后台返回的数据是',1,2,3'这种的，截取后，要去除第一个空格
                                if (temp_arr != null && temp_arr.length > 0) {
                                    temp_arr.splice(0);
                                    for (var i = 0; i < temp_arr.length; i++) {
                                        if (_self.fourCases_arr[num].key == temp_arr[i]) {
                                            _self.fourCases += _self.fourCases_arr[num].text + ' ';
                                            _self.fourCasesId += ',' + _self.fourCases_arr[num].key;
                                        }
                                    }
                                }
                            }
                            //多选，复选 三涉情况
                            for (var num in _self.threeInvolved_arr) {
                                var temp_arr = ret.data.threeInvolved.split(',');
                                //后台返回的数据是',1,2,3'这种的，截取后，要去除第一个空格
                                if (temp_arr != null && temp_arr.length > 0) {
                                    temp_arr.splice(0);
                                    for (var i = 0; i < temp_arr.length; i++) {
                                        if (_self.threeInvolved_arr[num].key == temp_arr[i]) {
                                            _self.threeInvolved += _self.threeInvolved_arr[num].text + ' ';
                                            _self.threeInvolvedId += ',' + _self.threeInvolved_arr[num].key;
                                        }
                                    }
                                }
                            }
                            //多选，复选 矫正小组人员组成情况
                            for (var num in _self.correctionDetail_arr) {
                                var temp_arr = ret.data.correctionDetail.split(',');
                                //后台返回的数据是',1,2,3'这种的，截取后，要去除第一个空格
                                if (temp_arr != null && temp_arr.length > 0) {
                                    temp_arr.splice(0);
                                    for (var i = 0; i < temp_arr.length; i++) {
                                        if (_self.correctionDetail_arr[num].key == temp_arr[i]) {
                                            _self.correctionDetail += _self.correctionDetail_arr[num].text + ' ';
                                            _self.correctionDetailId += ',' + _self.correctionDetail_arr[num].key;
                                        }
                                    }
                                }
                            }
                            for (var num in _self.whetherRecidivists_arr) {
                                if (_self.whetherRecidivists_arr[num].key == ret.data.whetherRecidivists) {
                                    _self.whetherRecidivists = _self.whetherRecidivists_arr[num].text;
                                    _self.whetherRecidivistsId = _self.whetherRecidivists_arr[num].key;
                                }
                            }
                            for (var num in _self.correctionGroup_arr) {
                                if (_self.correctionGroup_arr[num].key == ret.data.correctionGroup) {
                                    _self.correctionGroup = _self.correctionGroup_arr[num].text;
                                    _self.correctionGroupId = _self.correctionGroup_arr[num].key;
                                }
                            }
                            for (var num in _self.thereisFthecamp_arr) {
                                if (_self.thereisFthecamp_arr[num].key == ret.data.thereisFthecamp) {
                                    _self.thereisFthecamp = _self.thereisFthecamp_arr[num].text;
                                    _self.thereisFthecampId = _self.thereisFthecamp_arr[num].key;
                                }
                            }
                            for (var num in _self.isLouguan_arr) {
                                if (_self.isLouguan_arr[num].key == ret.data.isLouguan) {
                                    _self.isLouguan = _self.isLouguan_arr[num].text;
                                    _self.isLouguanId = _self.isLouguan_arr[num].key;
                                }
                            }
                            for (var num in _self.isMorecrimes_arr) {
                                if (_self.isMorecrimes_arr[num].key == ret.data.isMorecrimes) {
                                    _self.isMorecrimes = _self.isMorecrimes_arr[num].text;
                                    _self.isMorecrimesId = _self.isMorecrimes_arr[num].key;
                                }
                            }
                            for (var num in _self.releaseType_arr) {
                                if (_self.releaseType_arr[num].key == ret.data.releaseType) {
                                    _self.releaseType = _self.releaseType_arr[num].text;
                                    _self.releaseTypeId = _self.releaseType_arr[num].key;
                                }
                            }
