 ...
 */
apiready = function () {
  // 精神障碍人员 mobilementalDisorders.shtml?act=list_for_jsza
  // 邪教人员 mobileHeresy.shtml?act=list_for_falungong
  var apiCan = 'mobileHeresy.shtml?act=list_for_falungong';
  if (api.pageParam.item.name == '精神障碍人员') {
    apiCan = 'mobilementalDisorders.shtml?act=list_for_jsza';
  }
  ...
  var vue = new Vue({
    ...
    created: function () {
      ...
      var that = this;
      api.ajax({
        url: UICore.serviceUrl + 'mobile/' + apiCan + '&accountId=' + accountId + '&data=' + JSON.stringify(this.resultjson),
        method: 'post',
      }, function (ret, err) {
        api.hideProgress();
        if (ret.success) {
    ...
    methods: {
      ...
      loadBottom: function () { //上拉加载
        ...
        api.addEventListener({
          ...
          api.ajax({
            url: UICore.serviceUrl + 'mobile/' + apiCan + '&accountId=' + accountId + '&data=' + JSON.stringify(that.resultjson),
            method: 'post',
          }, function (ret, err) {
            api.hideProgress();
            if (ret.success) {
      ...
      query: function () {
        ...
        api.ajax({
          url: UICore.serviceUrl + 'mobile/' + apiCan + '&accountId=' + accountId + '&data=' + JSON.stringify(resultjson),
          method: 'post'
        }, function (ret, err) {
          api.hideProgress();
          that.isshow = false;
