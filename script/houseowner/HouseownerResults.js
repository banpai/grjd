...
apiready = function() {
    var vue = new Vue({
        ...
        created: function() {
            ...
            console.log(UICore.serviceUrl + 'mobile/mobileInterfaceForNewResident.shtml?act=query&accountId='+this.accountId+'&data=' + JSON.stringify(this.resultjson));
            api.ajax({
                url: UICore.serviceUrl + 'mobile/mobileInterfaceForNewResident.shtml?act=query&accountId='+this.accountId+'&data=' + JSON.stringify(this.resultjson),
                method: 'get',
            }, function(ret, err) {
                api.hideProgress();
                if (ret.success) {
            ...
            api.addEventListener({
                ...
                if (ret) {
                    ...
                    for(var num in _self.items){
                      if(_self.items[num].residentId==myJson.resident.id){
                        for(mykey in myJson.resident){
                          if(mykey=="houseid"){
                            _self.items[num]["houseid"] = myJson.resident[mykey];
                          }else if(mykey=="HOUSE_OWNER"){
                            _self.items[num]["houseOwner"] = myJson.resident[mykey];
                          }

                        }
                        for(mykey in myJson.refreshBuild){
                          _self.items[num][mykey] = myJson.refreshBuild[mykey];
        ...
        methods: {
            ...
            loadList: function() {
                ...
                console.log("下拉刷新: " + UICore.serviceUrl + 'mobile/mobileInterfaceForNewResident.shtml?act=query&accountId='+this.accountId+'&data=' + JSON.stringify(this.resultjson));
                api.ajax({
                    url: UICore.serviceUrl + 'mobile/mobileInterfaceForNewResident.shtml?act=query&accountId='+this.accountId+'&data=' + JSON.stringify(this.resultjson),
                    method: 'get',
                }, function(ret, err) {
            ...
            loadBottom: function() { //上拉加载
                ...
                api.addEventListener({
                    ...
                    console.log(UICore.serviceUrl + 'mobile/mobileInterfaceForNewResident.shtml?act=query&accountId='+this.accountId+'&data=' + JSON.stringify(that.resultjson));
                    api.ajax({
                        url: UICore.serviceUrl + 'mobile/mobileInterfaceForNewResident.shtml?act=query&accountId='+this.accountId+'&data=' + JSON.stringify(that.resultjson),
                        method: 'get',
                    }, function(ret, err) {
                        api.hideProgress();
                        if (ret.success) {
            ...
            query:function() {
                ...
                console.log(UICore.serviceUrl + 'mobile/mobileInterfaceForNewResident.shtml?act=query&accountId='+this.accountId+'&data=' + JSON.stringify(resultjson));
                api.ajax({
                    url: UICore.serviceUrl + 'mobile/mobileInterfaceForNewResident.shtml?act=query&accountId='+this.accountId+'&data=' + JSON.stringify(resultjson),
                    method: 'get',
                }, function(ret, err) {
                    api.hideProgress();
                    that.isshow = false;
